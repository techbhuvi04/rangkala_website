// Particle System — Canvas background animation
...
class ParticleSystem {
  ...
  updateParticle(p) {
    ...
    if (!p.isAmbient) {
      const dynamicCenterX = (this.canvas.width * 0.8) + (this.swayOffsetX * 150); 
      const dynamicCenterY = (this.canvas.height * 0.45) - (this.swayOffsetY * 150);
      
      const isHome = window.location.pathname === '/' || window.location.pathname === '';
      const roamFree = !isHome || (this.swayOffsetX === 0 && this.swayOffsetY === 0);

      if (!roamFree) {
        const cdx = p.x - dynamicCenterX;
        const cdy = p.y - dynamicCenterY;
        const distToCenter = Math.sqrt(cdx * cdx + cdy * cdy);
        const maxRadius = this.canvas.width * 0.12;

        if (distToCenter > maxRadius) {
          const pullStrength = (distToCenter - maxRadius) * 0.02;
          p.x -= (cdx / distToCenter) * pullStrength;
          p.y -= (cdy / distToCenter) * pullStrength;
        }
      } else {
